...
**摘要与备选模型**：全书/单章摘要**始终先用** `llm.model`（默认模型）。若回复无法解析为合法 JSON（例如模型输出 Markdown 而非 JSON），且配置了非空的 `llm.fallback_model`，会**自动用备选模型再请求一次**；两次都失败则仍写入 `abstracts/*.raw.txt` 并报错。`fallback_base_url` 留空表示与 `base_url` 相同。
...
| 目录 / 路径 | 说明 |
|------------|------|
| `book_resource/`（默认） | 原始整书 `.txt` 放置处；实际路径由 `config.json` 的 `dir.input_dir` 决定 |
| `book_chapters/<书名>/`（默认） | 拆分后的章节文件，如 `001_第一章_xxx.txt`；根目录由 `dir.output_dir` 决定 |
| `book_chapters/<书名>/abstracts/`（默认） | 每章摘要：`001.json`、`001.md`；失败时会有 `001.raw.txt`；`abstracts` 可由 `dir.summary_dir_name` 改名 |
| `book_chapters/<书名>/world/`（默认） | 世界状态快照（由 `world` 命令生成）；`world` 可由 `dir.world_dir_name` 改名 |
...
```bash
go run . serve        # 默认端口见 config.json 的 serve.default_port（一般为 8080）
go run . serve 9000   # 命令行指定端口，覆盖配置中的默认值
```
...
| 场景 | 命令 | 说明 |
|------|------|------|
| **从 .raw.txt 解析出 JSON 并生成 .json/.md** | `go run . raw2json [书目录]` | 对 `abstracts/*.raw.txt` 做 JSON 提取；成功则生成同名 `.json`、`.md` 并**删除**该 `.raw.txt`。省略书目录时使用 `config.json` 中的 `summary_demo_chapter_dir` |
...
| 目的 | 命令 | 说明 |
|------|------|------|
| **检查 Qdrant 是否正常** | `go run . qdrant` | 连接 `config.json` 中 `qdrant.host` / `qdrant.port`，创建集合并做一次写入+检索 |

向量维度须与当前 embedding 模型输出一致，在 **`config.json` 的 `qdrant.vector_size`** 中配置（默认 2048）。更换 embedding 模型后请同步修改该值；若与 Qdrant 已有集合维度不一致，程序会删除旧集合并按新维度重建（会清空该集合内数据）。
...
| 无子命令 | `go run .` | 仅拆分：把 `dir.input_dir` 下所有 `.txt` 拆到 `dir.output_dir`（默认即 `book_resource/` → `book_chapters/`） |
...
3. **摘要有一批 .raw.txt 想抢救**  
   ...
   若希望减少 `.raw.txt` 产生，可在 `config.json` 中配置 `llm.fallback_model`，让默认模型解析失败时自动换更强模型重试一次（见上文「摘要与备选模型」）。
